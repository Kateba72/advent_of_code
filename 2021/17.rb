...
def part1
  ...
  puts max_y_initial * (max_y_initial + 1) / 2
end
...
def part2
  ...
  puts x_initial.to_a.product(y_initial.to_a).filter { |x, y| velocity_covers?(x_range, y_range, Vector[x, y]) }.count
end
...
def get_input(test=false)
  input = test ? get_test_input(test) : get_aoc_input(2021, 17)
